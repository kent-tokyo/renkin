...
`renkin-forward enumerate --help`, or `renkin-forward benchmark --help` for\n\
subcommand options.";
...
fn main() -> Result<()> {
    ...
    {
        bail!(
            "unknown subcommand {subcommand:?}. Use 'predict', 'validate', 'enumerate', 'benchmark', '--help', or '--version'."
        );
    }
