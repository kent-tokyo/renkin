...
```
renkin/                          ← Cargo workspace ルート（整備予定）
├── Cargo.toml
├── src/                         ← renkin クレート（逆合成）
│   ├── lib.rs                   # ライブラリクレート
...
- [ ] ルートコストスコアリング（市販試薬の価格統合）
- [ ] Cargo workspace 整備 — `crates/renkin-forward/` 兄弟クレート追加
- [ ] `renkin-forward`：テンプレートベース順反応予測（reactants → products）
- [ ] `renkin-forward` を使った逆合成ルートの順反応検証（オプション機能）
