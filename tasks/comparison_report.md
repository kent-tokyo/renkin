...
chematic 互換。**depth 拡大とテンプレート増強は同程度の効果（各 +10pp 前後）** という知見。
...
次の一手（どちらも +10pp 前後が期待できる）
  A) depth=5 + top-500 テンプレート（314 ルール）の組み合わせ
  B) chematic #13/#14 修正（0.4.12 確認済み）を活かした SMIRKS ルール追加

競合水準
  ASKCOS: 41%（ルールベース最強クラス）
