...
- [ ] Batteries-included reranker distribution — `scripts/fetch_reranker_model.py` downloads/verifies `model.txt`/`frequency_table.json` from a GitHub Release asset; both are already attached to the `v0.22.0` release and fetchable today, pending this feature's own PR merging ([#101](https://github.com/kent-tokyo/renkin/issues/101))
