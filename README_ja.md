...
| **LightGBM candidate reranker**（Issue #101; CLIはv0.22.0で、Python面とbatteries-included配布はv0.23.0で出荷済み） | `--reranker-model model.txt --reranker-freq-table frequency_table.json`（CLI）または `reranker_model_path`/`reranker_freq_table_path`（Python `find_routes()`）— opt-inかつordering-onlyな仕組み: 同一ステップ内の候補を凍結済みLightGBMモデルで再順位付けし、テンプレート頻度bonusと同じスケール上のrank由来bonusとして表現する。生成される候補そのものは一切変更されず、探索順序のみが変わる。いずれかのフラグ/パラメータを省略した場合（デフォルト）はレガシー順序とbyte単位で完全一致する。model/tableのパスが不正な場合はrunを失敗させず、stderr警告付きでレガシー順序へfallbackする。C/C++依存なしのpure-Rustモデルリーダー。Paired 100-target route-searchゲート: `route_to_configured_stock` 16→20（+4/-0）。学習済み `model.txt` はどの公開パッケージにも同梱しない（学習に使ったUSPTO-50kデータのライセンスが上流で未文書化のため — 詳細は `docs/guides/open-source-retrosynthesis-comparison.md` の "Known gaps"）。`python3 scripts/fetch_reranker_model.py` で取得する（既にcommit/同梱済みの `frequency_table.json` も再検証のため一緒に取得） — v0.22.0 releaseに既に添付済みのGitHub Release assetからダウンロードし、二重のSHA-256検証を行う。 |
