...
class TestStockLeafValidation(unittest.TestCase):
    ...
    def test_leaf_claimed_stock_but_not_in_configured_stock(self):
        outcome = rg.normalize_renkin_route(renkin_single_step_route(), TARGET)
        # Configured stock (e.g. matched_stock mode) doesn't include benzoic acid.
...
class TestMassConservation(unittest.TestCase):
    def test_esterification_is_balanced_despite_water_byproduct(self):
        # Product has FEWER heavy atoms than the precursor sum (water is lost) -- must pass.
        outcome = rg.normalize_renkin_route(renkin_single_step_route(), TARGET)
        status, warnings = v.check_mass_conservation(outcome.graph)
        self.assertEqual(status, "balanced")
        self.assertNotIn(v.ELEMENT_IMBALANCE, warnings)

    def test_atom_materializing_from_nowhere_is_imbalanced(self):
        ...
        status, warnings = v.check_mass_conservation(outcome.graph)
        self.assertEqual(status, "imbalanced")
        self.assertIn(v.ELEMENT_IMBALANCE, warnings)
    ...
    def test_leaf_only_route_is_not_evaluable(self):
        ...
        status, warnings = v.check_mass_conservation(graph)
