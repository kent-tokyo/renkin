...
| 全件、top-500 + depth=5（進行中） | 463 | 314 | 5 | **≈49%**（13/50 チャンク速報） | 2026-06-22 |

### 競合比較

| ツール | 成功率 (top-1) | 評価条件 | 出典 |
|---|---|---|---|
| **RENKIN（depth=3、222 rules）** | **27.8%** | 全 4,907 件、beam=50、463 BB | 社内計測 |
| **RENKIN（depth=5、222 rules）** | **38.9%** | 全 4,907 件、beam=50、463 BB | 社内計測 2026-06-21 |
| **RENKIN（depth=3、314 rules）** | **38.2%** | 全 4,907 件、beam=50、463 BB | 社内計測 2026-06-21 |
| **RENKIN（depth=5、314 rules）** | **≈49%（速報）** | 13/50 チャンク完了時点 | 社内計測 2026-06-22 |
...
depth=5 + top-500（283 件）組み合わせ   → ≈49%（速報）      両者を重ねた相乗効果
```
...
組み合わせると相乗効果があり、ルールベースとして AiZynthFinder 水準（45-53%）を超えつつある。
...
| 弱み | 原因 | 対処（ロードマップ） |
|---|---|---|
| 成功率 ≈49%（速報、確定待ち） | depth=5+top-500 で競合水準に到達しつつある | depth=5 + top-500 確定後に評価 |
...
進行中
  463 BB / 314 ルール / depth=5  → ≈49%（13/50 チャンク速報、実行中）

次のアクション（ベンチマーク完了後）
  depth=5 + top-500 の確定値を取得 → comparison_report 最終更新
  chematic #13/#14 修正活用 → SMIRKS ルール追加でさらに +2-5%

競合水準
  ASKCOS: 41%（ルールベース最強クラス）←【速報では既に超過】
  AiZynthFinder: 45-53%（6M BB + 50k テンプレート）←【速報では下限に到達】
```
...
RENKIN は **テンプレート自動抽出と depth=5 の組み合わせにより、速報値で ≈49%** を記録。
ルールベース手法として AiZynthFinder 下限（45%）を超え、ASKCOS（41%）も上回りつつある。
