...
> 最終更新: 2026-06-22（depth=5 + top-500 確定値）  
> バージョン: renkin v0.1.0
...
| **RENKIN（depth=5、314 rules）** | **47.2%** | 全 4,907 件、beam=50、463 BB | 社内計測 2026-06-22 |
...
| 弱み | 原因 | 対処（ロードマップ） |
|---|---|---|
| 成功率 47.2%（AiZynthFinder 最大 53% に対し -6pp） | NN スコアリング・大規模 BB なし | GNN 統合・BB 拡充（長期） |
...
達成済み
  ...
  463 BB / 314 ルール / depth=5  → 47.2%（全 4,907 件）  ← 確定 ✅

競合水準（参考）
  ASKCOS: 41%  ← RENKIN が超過
  AiZynthFinder: 45-53%  ← RENKIN が下限を超過（47.2% > 45%）

次のアクション
  chematic #18 修正（bracket atoms）→ split_fragments ワークアラウンド不要に
  chematic #19 修正（parse_smarts atom-map）→ テンプレート検証が高速化
  chematic #20（立体化学 SMIRKS）→ Phase 15 対応
  top-1000 テンプレート試験 → さらなる向上の余地を確認
```
...
RENKIN は **テンプレート自動抽出（rdchiral）と depth=5 の組み合わせにより 47.2%** を達成。
ルールベース手法として AiZynthFinder 下限（45%）を超え、ASKCOS（41%）も上回った。
...
6M BB・50k テンプレートを持つ AiZynthFinder 下限に到達した。
