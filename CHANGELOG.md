...
- `renkin-forward benchmark --strict` now hashes its executable with a fixed-
  memory streaming reader, so Linux debug binaries larger than the external
  text-input limit retain complete reproducibility provenance.
...
- Comparison manifests now reject excessive JSON nesting before
...
- Generated sample manifests and frozen lists are now written atomically after
...
- MCP Pareto objective specifications now reject wrong-typed values instead of
...
- Added matching `avoid_building_blocks` and `require_building_blocks` filters
