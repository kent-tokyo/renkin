...
### Fixed
- **`renkin` lib** — `ReactionStep::atom_economy` (MW(target) / Σ MW(precursors) × 100) is no longer silently clamped to 100.0 when the raw ratio exceeds it ([#79](https://github.com/kent-tokyo/renkin/issues/79)): a route whose precursor set supplies less mass than the target needs (a target-atom-loss signal — e.g. a template misapplication that drops a fragment) previously reported an atom economy indistinguishable from a genuinely perfect route, since both clamped to `100.0`. A new `atom_economy_status` field (`normal` / `above_physical_range` / `not_evaluable`, always serialized) and `atom_economy_raw_percent` (the unclamped ratio, populated whenever both molecular weights are computable) are added to every step; `atom_economy` itself is now `None` — never a fabricated substitute — whenever `atom_economy_status` isn't `normal`. `renkin-forward`'s human-readable route explanation now flags an `above_physical_range` step by name ("target-atom loss suspected in step N"). Purely a diagnostic/reporting fix: `atom_economy` was already excluded from search-time ranking before this change and remains so; route search, selection, and JSON route hash are unaffected.
