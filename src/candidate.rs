...
pub struct CandidatePool {
    ...
    pub target_smiles: String,
    /// Deterministic per-rule accounting for this proposal call. Entries are
    /// in the proposal mode's selection order and retain rules that were
    /// filtered by target elements, so a zero-contribution rule is never
    /// invisible in coverage diagnostics.
    pub rule_stats: Vec<RuleCandidateStats>,
...
/// Per-rule one-step proposal accounting for a [`CandidatePool`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct RuleCandidateStats {
    /// Position in the proposal mode's selected-rule order.
    pub source_rank: usize,
    /// Human-readable rule name, retained alongside the stable template ID.
    pub rule_name: String,
    /// Stable rule/template identity used for provenance joins.
    pub template_id: String,
    /// Whether the target contains every element required by this rule.
    pub element_eligible: bool,
    /// Raw candidates generated by this rule before precursor-set merging.
...
pub(crate) fn raw_propose(
        ...
        && element_accounting_policy == ElementAccountingGatePolicy::Off
    {
        ...
        // Rayon setup/synchronization costs dominate when bond-index
        // retrieval leaves only a handful of applicable rules. Keep the
...
impl<'a> CandidateProposalContext<'a> {
    ...
    pub fn propose_one_step(
        ...
        let rule_stats: Vec<RuleCandidateStats> = active_rules
            .iter()
            .enumerate()
            .map(|(source_rank, rule)| {
                let element_eligible = rule.rule.required_elements == 0
                    || (target_elem_mask & rule.rule.required_elements
                        == rule.rule.required_elements);
                RuleCandidateStats {
                    source_rank,
                    rule_name: rule.rule.name.clone(),
                    template_id: rule.rule.template_id.clone(),
                    element_eligible,
                    raw_candidates_generated: raw_counts_by_rank
                        .get(&source_rank)
                        .copied()
                        .unwrap_or(0),
                }
            ...
            .collect();
        let producing_rule_ranks: std::collections::HashSet<usize> = rule_stats
            .iter()
            .filter(|rule| rule.raw_candidates_generated > 0)
            .map(|rule| rule.source_rank)
            .collect();
...
mod tests {
    ...
    fn candidate_pool_stats_make_raw_and_dedup_counts_explicit() {
        ...
        assert_eq!(
            pool.rule_stats
                .iter()
                .filter(|rule| rule.element_eligible)
                .count(),
            pool.stats.rules_attempted
        );
    }
    ...
    fn candidate_pool_stats_separate_mode_and_element_filtering() {
        ...
        assert_eq!(
            pool.rule_stats[0].raw_candidates_generated,
            pool.stats.raw_candidates_generated
        );
        assert!(pool.rule_stats[0].raw_candidates_generated > 0);
        assert_eq!(pool.rule_stats[1].source_rank, 1);
        assert!(pool.rule_stats[1].element_eligible);
        assert_eq!(pool.rule_stats[1].raw_candidates_generated, 0);
        assert_eq!(pool.rule_stats[2].source_rank, 2);
        assert!(!pool.rule_stats[2].element_eligible);
        assert_eq!(pool.rule_stats[2].raw_candidates_generated, 0);
    }
    ...
    fn assert_context_api_matches_legacy_api(
        ...
        assert_eq!(legacy_pool.rule_stats, context_pool.rule_stats);
    }
