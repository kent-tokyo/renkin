...
服务器会自动检测工作目录下的 `data/building_blocks.smi` 与 `data/templates_extracted_5000.smi`；若未找到，则回退到内置的 `DEFAULT_BUILDING_BLOCKS` / `default_rules()` 默认值（根据 `ChemEnv::bb_count()`，为 152 种去重起始原料、27 条人工编写规则——已于 2026-08-22 核实，`aryl_amine_retro`（issue #77）被移除后数量从 28 降为 27；此前此处曾记载过"509 种起始原料 / 20 条规则"的数字，但未经核实）。
