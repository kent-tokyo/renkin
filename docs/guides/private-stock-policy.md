...
`lead_time_days`, `hazard`, `region`, and `available`. The policy is versioned JSON:
...
`region_not_allowed`, `region_blocked`, or `prohibited_substance`. A missing
...
`region` is also exact and case-sensitive. When `allowed_regions` is non-empty,
an offer without a matching region is rejected; `blocked_regions` always wins.
