...
- **`renkin` CLI** — `--open-state-diagnostics` (implies `--search-diagnostics`): nine new `search_diagnostics` counters (`open_state_candidates_considered`, `open_state_unique_inserted`, `open_state_dominated_skipped`, `open_state_better_replacements`, `stale_open_nodes_discarded_on_pop`, `stale_open_nodes_removed_before_beam_prune`, `peak_unique_open_states`, `peak_raw_heap_nodes`, `peak_duplicate_open_nodes`) computing what an *open-state dominance* decision would be for every non-terminal candidate push, without acting on it — competitive-diagnostics program Phase 2A, offline use only. Distinct from the existing `candidates_after_cross_template_dedup` counters above, which only ever see one expansion's own proposals: this tracks a depth-scoped, exact-frontier `StateKey` (sorted canonical SMILES with multiplicity) across the *whole* open heap, to measure the same-logical-state-pushed-from-different-derivation-paths crowd-out mechanism documented in the Phase 1D/1E candidate trace (`data/phase1c_diagnostics/L1541_candidate_narrative.md`) — a mechanism the same-parent dedup counters cannot see by construction. Gated by a new `SearchConfig::open_state_diagnostics: bool` field (`false` by default, zero extra allocation when off); when on, nothing is skipped, replaced, or marked stale — `stale_open_nodes_*` are always `0` in this mode by construction (nothing is ever superseded when nothing is ever acted on). Validated against L1541/L984/L1640/L4092 (beam=100, Conservative, shared_stock): 27-34% of all candidate pushes land on an already-generated state across all four targets, not just L1541 — see `data/phase2a_open_state_diagnostics/phase2a_findings.md` (local, untracked). **Superseded below (Phase 2 Round 2): this 27-34% figure was measured against the ghost-record bug fixed by the Round 2A-2D lifecycle correction and needs re-measurement; treat it as a withdrawn preliminary number, not a current one, until Round 2F/2G re-runs it.**
- **`renkin` CLI** — `--open-state-dominance` (EXPERIMENTAL, implies `--search-diagnostics`; default off): acts on the decision `--open-state-diagnostics` only counts — competitive-diagnostics program Phase 2B-2D, [#101](https://github.com/kent-tokyo/renkin/issues/101). A candidate whose depth-scoped, exact-frontier `StateKey` already has a recorded path cost `g` <= its own is never pushed onto the open heap (ties keep whichever was generated first, deterministic); a candidate with a strictly lower `g` replaces the record, and the node that previously held it becomes stale — actively removed from the heap before every `beam_prune` call (so it cannot consume beam-width capacity a genuinely distinct candidate should get instead), with a pop-time discard as a backstop that a correct pre-`beam_prune` filter should make unreachable in practice. Terminal states (`n_unsolved == 0`) are always excluded from dominance, preserving the existing behaviour of returning multiple distinct routes that converge on the same building-block frontier. Gated by a new `SearchConfig::open_state_dominance: bool` field (`false` by default); with the flag off, verified byte-identical route/stats output against a config that never sets any `open_state_*` field at all. `StateKey` uses full structural equality (sorted canonical SMILES with multiplicity, exact depth) as the actual map key, not a bare hash -- correctness never depends on hash quality, unlike the pre-existing `state_hash`-based closed set (see `state_key_exact_comparison_survives_hash_collisions`).
- **`renkin` CLI** — Phase 2 Round 2 fix (still experimental, still default off): `open_state_best`'s records are now lifecycle-correct. Previously, a node evicted by `beam_prune` on pure f()-score grounds (not superseded by a `Replaced` verdict) left a *ghost* record behind — a later, independent regeneration of the exact same `StateKey` could be incorrectly dominated by a state that was never actually explored (evicted from the beam, not superseded). Fixed by giving every open-state record a unique generation token (`OpenStateRecord.token: u64`) alongside its `best_g`, so liveness is decided by token equality, never by comparing `g` values alone (two records can coincidentally share a `g`, but never a token) — see `is_live_open_state_representative`. `beam_prune`'s return type gained a third element, `evicted_open_state_tokens`, so its caller can reconcile any evicted live representative's record out of the map (a new `open_state_records_dropped_on_beam_eviction` counter tracks this). As a consequence, `peak_unique_open_states`/`peak_raw_heap_nodes`/`peak_duplicate_open_nodes` now measure *currently-open* state, not *ever-seen* state, in both `--open-state-diagnostics` and `--open-state-dominance` modes (ghost cleanup is pure bookkeeping against the auxiliary map, never a heap mutation, so it is safe and byte-identical-preserving under diagnostics-only too). New regression test `open_state_dominance_ghost_reentry_after_beam_eviction` reproduces the bug end-to-end through a real `beam_prune` call. Default-off byte-identical-to-legacy behaviour re-verified after this change.
