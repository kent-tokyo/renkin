#[cfg(test)]
mod tests {
    use crate::chem_env::{ChemEnv, mol_from_smiles, to_canonical, apply_retro, default_rules};

    const SIMPLE_UNSOLVED: &[(&str, &str)] = &[
        ("[CH3]N([CH3])[CH2][CH]1[CH2][CH2][CH2][CH2]C1=O", "NMe2-ketone"),
        ("O=[CH]c1[cH][cH][cH]c(-c2[cH][cH][cH]o2)n1", "pyridine-furan-biaryl"),
        ("[CH3][NH]c1[cH]c([CH3])c(OC([CH3])=O)c([CH3])c1[CH3]", "aryl-amine-ester"),
        ("[CH3]C(=O)[CH2]c1[cH][cH]c2c([cH]1)O[CH2]O2", "ketone-arene"),
        ("[CH3]c1[cH]nc([CH3])c([CH3])c1[Cl]", "chloro-methylpyridine"),
    ];
    ...
    fn trace_pipeline() {
        ...
        for (smiles, desc) in SIMPLE_UNSOLVED {
            ...
            for rule in &rules {
                ...
                if precursors.is_empty() { continue; }
                any_rule_fired = true;
                println!("  RULE [{}]:", rule.name);
                for (i, set) in precursors.iter().enumerate().take(3) {
                    let parts: Vec<String> = set.iter().map(|p| {
