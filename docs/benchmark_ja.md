...
### 最新結果 (v0.1.3) — depth=5, beam=100, 5,000 extracted templates

| 設定 | 解決数 | 成功率 | 平均時間 | 実行環境 |
|------|--------|--------|----------|----------|
| depth=5, beam=100, 5,000 templates | **3,826 / 4,907** | **78.0%** | **2,775 ms/mol** | Apple M-series, 8 スレッド |

建築ブロック: 480 種類の手選定市販試薬（デフォルトセット）
...
v0.1.3 では、ニューラルネットワーク（Phase B）を使わずに Pure Rust のみで同等の精度を beam=100 で達成しました。

### 他システムとの比較

| システム | Top-1 | 在庫 | テンプレート数 | 備考 |
|---------|-------|------|-------------|------|
| **RENKIN v0.1.3** | **78.0%** | **480 BBs** | **5,000** | Pure Rust、C++ 依存なし |
