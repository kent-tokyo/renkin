...
### Fixed
- **`renkin` lib** — `[#N]`/`[#N:map]` bare atomic-number SMARTS primitives in extracted templates (e.g. `[#7:2]`, "any nitrogen, aromaticity unspecified") no longer silently fail at *apply* time in either retrosynthesis search or `renkin-forward` ([#88](https://github.com/kent-tokyo/renkin/issues/88)). Root cause: `chematic::rxn::run_reactants`/`parse_reaction` parse a SMIRKS through `chematic-smiles`'s SMILES grammar, which requires a concrete element symbol — SMILES has no spelling for "any isotope of this element" — while `chem_env::load_rules_from_file`'s load-time validation uses the SMARTS-capable `parse_smarts`, which accepts `[#N]` fine. This meant load-time success gave no signal about apply-time success: `chem_env::apply_retro` silently returned zero precursors for every molecule via an `.unwrap_or_default()` on the parse error, and `renkin-forward`'s `reverse_smirks_validated` independently hit the identical parser for the same reason. Confirmed empirically (not just by reading the parser source) with a standalone reproduction against real chematic 0.10: applying `data/templates_extracted.smi`'s line 21 (`[#7:2]:[c:1](-[NH:4]-[c:5]):[#7:3]>>Cl-[c:1](:[#7:2]):[#7:3].[NH2:4]-[c:5]`, freq 167) to 2-anilinopyrimidine (a real molecule containing the matched substructure) failed with `invalid bracket atom at position 1: missing element symbol` in both directions. Scope: 217/500 templates in the frozen benchmark corpus (`data/templates_extracted.smi` / `_500.smi`, ~43%), 3,461/5,004 in RENKIN's general-purpose default template file (`_5000.smi`, ~69%, and not limited to N/O/S — also B/Si/P/Se/Sn wildcards) were affected in both directions this whole time, including during the already-reported/merged Issue #66 500-target comparison.
  - `[#N]` genuinely doesn't specify aromaticity, so the fix (`chem_env::expand_hash_atom_variants`, called from `load_rules_from_file`) never guesses a single reading: each distinct `[#N]`/`[#N:map]` atom is expanded into every aromatic/aliphatic variant, and only the variants that independently re-parse via `chematic::rxn::parse_reaction` are kept as usable `RetroRule`s — nothing is asserted about which reading is chemically correct beyond what actually parses. The same atom-map appearing on both sides of `>>` is forced to the same choice (an inconsistent per-side element is treated as unexpandable, not guessed at); a bracket combining `#N` with any other primitive (e.g. `[#7;+0]`) is likewise left unexpanded, unchanged from today's pre-fix behavior for that one template (not a regression — just not fixed by this change). Combinatorial expansion is capped at 16 variants per template with an explicit `eprintln!` warning naming the template and the true combination count whenever the cap binds (never a silent truncation); the frozen 500-template corpus never needs more than 8 (`k=3`).
  - Every expanded variant keeps the **original** template's `template_id` (hash of the un-rewritten SMIRKS), so template-keyed metadata generated against the checked-in corpus — notably the ring-context guard's sidecar (`data/ring_context_metadata_500.json`, [#72](https://github.com/kent-tokyo/renkin/issues/72)/[#242](https://github.com/kent-tokyo/renkin/pull/242)) — still resolves them under `Disabled`/`AuditOnly`. **Known limitation**: under `Conservative`/enforced ring-context policy, a template_id lookup miss fails closed (`RingContextDiagnostics::templates_missing_metadata`, zero precursors) for any genuinely *new* variant the sidecar wasn't generated against — since expansion reuses the original template_id rather than minting new ones, this only matters for templates whose original entry was already `Conservative`-gated in a way the shared ID doesn't resolve cleanly; regenerating the sidecar to formally cover expanded variants is a follow-up, not bundled here.
  - Corpus-level effect on `data/templates_extracted.smi`: 500 raw lines now produce 865 `RetroRule`s (283 unchanged single rules + 217 hash-atom lines expanding to 582 validated variants, exactly matching the corpus's measured 155×2 + 56×4 + 6×8 distinct-hash-atom-count distribution), spanning all 500 distinct `template_id`s (locked in by `reverse_smirks_validated_extracted_templates_accept_reject_partition_is_stable`/`parse_hint_template_extracted_templates_accept_reject_partition_is_stable`) — all 865 now pass `reverse_smirks_validated` (previously 283/500, 217 rejected).
  - **Measured practical impact, stated precisely (no overclaim)**: a real-data 100-target native-stock RENKIN-only before/after re-run (`scripts/compare_run.py`, same sample/templates/stock as the existing 100-target benchmark) shows `route_found_rate` at 16→15 (`Disabled`) and 14→14 (`Conservative`) — within the noise of this sample size, not a demonstrated coverage gain. Per-target inspection confirms this: **zero** of the solved routes in either post-fix run actually use a hash-atom-expanded template; the handful of individual targets that flipped found↔not-found did so via unrelated templates, consistent with beam-search competition shifting when the rule set grows, not with this fix directly contributing a route in this specific sample. This does not contradict the fix's correctness (verified independently via a real end-to-end `apply_retro` regression test reproducing the exact 2-anilinopyrimidine → 2-chloropyrimidine + aniline decomposition) — it means this particular 100-target native/402-compound-stock sample doesn't happen to need these specific templates to solve any of its targets. The fix is justified on correctness grounds (a silently-broken template is a real defect independent of any one benchmark's sensitivity to it); a coverage benefit, if any, is not established by this measurement and would need a larger and/or heterocycle-heavier sample (or the production `_5000.smi` file, where the affected fraction is much larger) to detect.
  - 8 new unit tests (`chem_env::tests::hash_atom_*`) cover: no-op on ordinary SMIRKS, real-corpus expansion with re-parse validation, atom-map consistency enforcement across `>>`, rejection of an inconsistent same-atom-map element, rejection of a combined primitive, explicit visible capping on a large combinatorial space, `template_id` sharing across variants via `load_rules_from_file`, and the end-to-end `apply_retro` regression above. The pre-existing `reverse_smirks_validated_still_rejects_multi_condition_smarts`/`..._still_rejects_recursive_smarts` fixtures are unchanged and still pass, confirming the fix stayed narrow to bare atomic-number primitives.
