...
- **`renkin` lib** — `chem_env::apply_retro_call_count()`/`reset_apply_retro_call_count()`: an always-on (one relaxed atomic increment per call, no feature gate) counter of total `apply_retro` invocations, for the gate above and any future regression check

### Fixed
- **`renkin` lib** — partially addresses the `apply_retro`/`run_reactants` performance regression identified between chematic 0.4.25 and 0.4.30 (root-caused to a redundant `canonical_smiles()` rewrite, see `artifacts/perf_root_cause/`): `chematic` is now pinned to git commit [`97c87e3`](https://github.com/kent-tokyo/chematic/commit/97c87e36951ae4b316bb893d434292f41ba65968) (workspace `0.7.1`, unreleased to crates.io) instead of the crates.io `0.6.0` release, picking up chematic's own upstream fix for the redundant write. **This is a partial fix**, matching chematic's own characterization: on a 30-target gate (see `artifacts/perf_root_cause/fix_summary.md`), aggregate wall-clock improved ~16% with zero correctness impact (`apply_retro_calls` identical per target, solved status unchanged, zero errors), but the single worst-case target (a highly symmetric structure) was measured 12% *slower*, and p95 improved only 4.6% — the larger combinatorial canonicalization cost on symmetric molecules remains unfixed upstream (tracked at [chematic#193](https://github.com/kent-tokyo/chematic/pull/193), draft, unmerged). A full-sub-crate pin back to chematic `0.4.22` was evaluated and rejected: it reproduces the old speed but reintroduces a genuine SMILES-parser strictness regression (accepts malformed SMILES that `0.4.30`+ correctly rejects) — this repo does not trade validation strictness for speed. The formal 4,907-target Step 0 remeasurement remains not run.
