...
- [ ] Coverage mode (`--search-mode coverage`, [#101](https://github.com/kent-tokyo/renkin/issues/101)) — opt-in Stage-1/Stage-2 template-count escalation, addressing the candidate-generation coverage gap below. CLI/Python surface merged to `master`, not yet tagged/released: gated on one remaining one-shot formal-TEST confirmation run (`data/coverage_mode_formal_test/protocol.md`, pre-registered, not yet executed). See the Key Features table above for the shipped-so-far surface
