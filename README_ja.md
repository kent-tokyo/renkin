...
>
> **現在の状況:** 修正は2件のPR（`fix/forward-validation-graph-rule-blindspot`、`fix/aryl-carboxylation-retro-ester-overmatch`）としてland済み。加えて、手書きSMIRKSルール全件について同種のバグがないか手動監査を実施し、他に原子消失バグは見つからず（発見内容は回帰テストとして固定済み）。USPTO-50k全4,907件の再計測（修正版バイナリ）を実行中で、完了後にこの注記を正式な訂正値へ置き換えます。進捗は `tasks/todo.md`（Phase 31）を参照。
