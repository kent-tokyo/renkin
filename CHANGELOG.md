...
- **`scripts/fetch_reranker_model.py`** ([#101](https://github.com/kent-tokyo/renkin/issues/101)) — downloads the frozen `model.txt`/`frequency_table.json` from a GitHub Release asset and SHA-256 verifies before use. Two manifests, not one: `freeze_manifest.json` (training-time provenance) gives a whole-file hash for `model.txt` but only an inner-`table`-data hash for `frequency_table.json` (computed before `phase3e_export_frequency_table.py` wraps it in `_purpose`/`entries`/`table` keys) — a real gap caught during this work's own testing, since that alone can't catch a wrong/truncated/corrupted download of `frequency_table.json`. New `data/phase3e_reranker_training/release_asset_manifest.json` adds the missing whole-file check for it, so both files now get an unambiguous "SHA-256 verified download" guarantee. No new dependency: shells out to `curl`, matching this repo's existing `scripts/fetch_chembl_approved.py` convention. `model.txt` is deliberately not bundled into the crates.io/PyPI/npm packages themselves — its USPTO-50k training data's license is undocumented upstream (see `docs/guides/open-source-retrosynthesis-comparison.md`'s "Known gaps") — so this script is the "batteries-included" path instead: one command, downloaded from a versioned release asset with cryptographic provenance, rather than silently bundling a research-provenance artifact into an MIT-licensed package. (`frequency_table.json` is already committed/bundled — this script re-verifies it too for a single consistent command, not because it was otherwise unavailable.) `model.txt`/`frequency_table.json` are live as assets on the existing `v0.22.0` GitHub Release (the reranker's own release), independently confirmed via GitHub's own server-side asset digest; a future correction ships as a new release version rather than overwriting these in place.
