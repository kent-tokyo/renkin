...
| **LightGBM candidate reranker**（実験的、Issue #101） | `--reranker-model model.txt --reranker-freq-table frequency_table.json` — opt-inかつordering-onlyな仕組み: 同一ステップ内の候補を凍結済みLightGBMモデルで再順位付けし、テンプレート頻度bonusと同じスケール上のrank由来bonusとして表現する。生成される候補そのものは一切変更されず、探索順序のみが変わる。いずれかのフラグを省略した場合（デフォルト）はレガシー順序とbyte単位で完全一致する。model/tableのパスが不正な場合はrunを失敗させず、stderr警告付きでレガシー順序へfallbackする。C/C++依存なしのpure-Rustモデルリーダー。runtime paired-gateの結果が出るまで「実験的」タグを外さない。 |
...
- [x] RETROSPECT 着想のオフライン候補リランキング基盤 — candidate proposal/selection の分離、feature schema v1、manifest v2、leakage-safe な train/val/test スプリット、7つの決定的 baseline arm + 学習済み ranker arm、paired bootstrap + オフラインゲートツール（[#59](https://github.com/kent-tokyo/renkin/pull/59)）
- [x] オフライン候補リランカーを実データで学習・ゲート通過（[#101](https://github.com/kent-tokyo/renkin/issues/101) Task 35）— 実USPTO-50kラベルでLambdaMARTモデルを学習、VAL screening gate通過（top1 +11.7pp・MRR +11.3pp・top10 +9.3pp、bootstrap CI確認済み）、モデルをfreezeし、frozen modelに対し formal 4,903-target TEST評価を一度だけ実施しPASS（top1 +12.7pp・MRR +11.9pp・top10 +9.1pp — VALと同程度の改善幅でoverfitting兆候なし）。**オフラインのcandidate-rankingゲートのみ — route searchへの統合はまだ**（下記「進行中」参照）
- [ ] （進行中）学習済み候補リランカーのruntime統合（[#101](https://github.com/kent-tokyo/renkin/issues/101) Task 35）— 実装・テストは完了。残るのはpaired 100-target route-searchゲート（reranker ON vs OFF、同一commit/corpus/stock/templates/budget）のみ。結果が出るまでrelease/tagは行わない
