...
| **RENKIN Bridge / `audit-route`** | `renkin audit-route route.json [--stock stock.smi] [--output human\|json]` — ツール非依存のroute audit: 構造整合性・stock・宣言済み反応のforward replay検証を、それぞれ独立に `pass`/`fail`/`not_evaluable` で報告し、route全体は `pass`/`fail`/`partial` で判定。v0.25.0時点ではRENKIN-native route JSONのみ正式対応。audit modelそのものはツール非依存として設計されており、今後別のsource adapterを追加可能 |
...
- [x] **RENKIN Bridge Preview**（`renkin audit-route`、v0.25.0で出荷）— *Keep your current workflow. Add an auditable second opinion.* ツール非依存のroute audit model: 構造整合性・stock・宣言済み反応のforward replay検証を、それぞれ独立に `pass`/`fail`/`not_evaluable` で報告し、route全体は `pass`/`fail`/`partial` で判定——boolean へ暗黙に握り潰さない。audit modelそのものはツール非依存だが、v0.25.0で出荷するのはRENKIN-native route JSON向けの最初のアダプタのみ。`renkin audit-route route.json --stock stock.smi --output json` で、ファイル内の全routeを監査し、machine-readableな1つのreportへ集約する
...
- [ ] グラフルール拡張 — sulfonamide / carbamate / urea cleavage（family毎に1 PR、それぞれbenchmark差分付き）
