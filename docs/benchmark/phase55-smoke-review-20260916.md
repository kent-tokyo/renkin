...
| Timing / resume | Fresh process per target; final sweep time was last invocation only; run required manual continuation | `compare_run.py` now records each normally completed invocation in the manifest and reports its durable cumulative wall time separately from this invocation. New rows use `planner_timing_v1`: common process wall-clock, post-exit adapter audit time, and optional tool-reported search time are separate fields. The latter is diagnostic, not a cross-tool metric. An externally killed invocation remains an explicit unrecorded gap; effective resource limits and supervised continuation still need protocol coverage |
