...
| Resources | RENKIN runs on macOS with `RLIMIT_AS_unavailable`; AiZynthFinder uses Docker CPU/memory limits | Verify effective thread/CPU and memory limits under a comparable execution environment; equal declared numbers are insufficient |
