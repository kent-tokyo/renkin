...
> ⚠️ **下記の項目1は上記の失敗要因分析（修正前のpre-31.11/31.12計測、修正済みルールセットでは未再計測）を根拠としています。** 一般的な方向性としては引き続き有効ですが、「主要なレバー」という位置付けは再確認された根拠ではありません。

1. **Cascade search** — 未解決ターゲットをより高いbeam/depthで再実行（`--depth 7 --beam-width 300`）。（無効化済み・修正前の）失敗要因分析では、これが主要なレバーであることが示されていました。
