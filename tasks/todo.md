...
#### Phase B ONNX テンプレート関連性モデル（次の重点フェーズ、+5〜10% 期待）
- [ ] `scripts/train_template_scorer.py` で MLP 訓練（Morgan FP → template prob）
- [ ] `ort` クレートで Rust ONNX 推論（feature = `nn-scoring`）
- [ ] A* ヒューリスティックに分子固有テンプレート選択を統合
- [ ] 効果確認: +5〜10%（分子固有のテンプレート選択による向上）
- ※ 旧 18.4（GNN スコアリング）の後継として位置付け

### ポジション目標
```
現在: 78.0%（全件確定）/ chematic 0.4.16 再計測中 / CF3 BB +24件で計測中
目標: 80%+（Phase B 後）
```
